...
thread_local! {
    /// 全局版本号：每次信号"值实际变化"时递增，供 memo 短路判断。
...
/// 2. 若观察者需要快照（memo），调用 `build` 惰性构建快照与再读取器，
///    交给观察者的 `collect`（memo 借此维护 relay map）。
///
/// `build` 只在"存在需要快照的观察者"时才执行，避免 effect 等纯监听路径
/// 每次读取都分配 `Rc` 与 `Box`。
///
/// 此函数可能在观察者执行（`add_fun`/evaluate）期间被调用，因此只做
/// 短生命周期借用取出 current 强引用，之后再在全局 borrow 之外完成
/// listener 注册与 collect，避免 `RefCell` 重入。
pub(crate) fn register_dep<B>(dep_id: NodeId, listeners: Rc<RefCell<Vec<NodeId>>>, build: B)
where
    B: FnOnce() -> (Box<dyn ValBox>, ReGet),
{
    ...
    if let Some(cur) = current {
        ...
        if cur.wants_dep_snapshot() {
            let (snapshot, reget) = build();
